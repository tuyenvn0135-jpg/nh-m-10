        ...
        <?php if($products && $products->num_rows > 0): ?>
            <?php while($row = $products->fetch_assoc()): ?>
                <div class="bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition flex flex-col justify-between text-xs font-medium" data-aos="fade-up">
                    ...
                    <div class="p-4 pt-0">
                        <a href="/kingfisher/MID/cart.php?action=add&id=<?php echo $row['id']; ?>" class="w-full block text-center bg-[#0A2540] text-white py-2 rounded text-[11px] font-bold uppercase hover:bg-blue-900 transition">🛒 Thêm giỏ hàng</a>a>
                    </div>
                </div>
            <?php endwhile; ?>
        <?php endif; ?>
